<ul
    ...
    <li
        class="text-gray-400 hover:bg-gray-100 px-4 py-2.5 rounded-lg text-sm font-bold cursor-pointer flex items-center
          {{ Route::currentRouteName() == 'customer.tracking.pending' ? 'border-b-2 border-blue-500 text-blue-600' : '' }}">
        <a onclick="window.location.href = '{{ route('customer.tracking.pending') }}'">Pending</a>
    </li>
    <li
        class="text-gray-400 hover:bg-gray-100 px-4 py-2.5 rounded-lg text-sm font-bold cursor-pointer flex items-center
          {{ Route::currentRouteName() == 'customer.tracking.processed' ? 'border-b-2 border-blue-500 text-blue-600' : '' }}">
        <a onclick="window.location.href = '{{ route('customer.tracking.processed') }}'">Processed</a>
    </li>
    <li
        class="text-gray-400 hover:bg-gray-100 px-4 py-2.5 rounded-lg text-sm font-bold cursor-pointer flex items-center
          {{ Route::currentRouteName() == 'customer.tracking.receiving' ? 'border-b-2 border-blue-500 text-blue-600' : '' }}">
        <a onclick="window.location.href = '{{ route('customer.tracking.receiving') }}'">To Receive</a>
    </li>
    <li
        class="text-gray-400 hover:bg-gray-100 px-4 py-2.5 rounded-lg text-sm font-bold cursor-pointer flex items-center
          {{ Route::currentRouteName() == 'customer.tracking.delivered' ? 'border-b-2 border-blue-500 text-blue-600' : '' }}">
        <a onclick="window.location.href = '{{ route('customer.tracking.delivered') }}'">Completed</a>
    </li>
    <li
        class="text-gray-400 hover:bg-gray-100 px-4 py-2.5 rounded-lg text-sm font-bold cursor-pointer flex items-center 
          {{ Route::currentRouteName() == 'customer.tracking.cancelled' ? 'border-b-2 border-blue-500 text-blue-600' : '' }}">
